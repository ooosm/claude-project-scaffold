...
- **fix**: §7-A가 코드펜스·백틱을 걷어낸 뒤 판정하도록 수정(§4·§7-C와 같은 제외 규칙).
  changelog는 과거 문구를 인용하는 일이 잦은데 인용을 완료 언급으로 세고 있었다 — v0.2.0을
  끊는 도중 실제로 재현됐다(DEC-016 오탐 대응을 기록한 `fix` 항목이 그 사례를 인용해 자기
  자신을 잡았다). 회귀 케이스로 고정.
