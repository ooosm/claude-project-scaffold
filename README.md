...
| 방식 | 무엇인가 | 기존 프로젝트(brownfield) 적용 | 강점 | 약점 |
|------|----------|--------------------------------|------|------|
| **스캐폴드 (이 레포)** | 룰·문서·워크스페이스 골격을 담은 템플릿 폴더를 복사해 시작 | ✅ **가능** — 룰+구조물을 실제 파일로 함께 가져와 선별 병합 | 이식성 높음, 팀·타 머신 공유 용이, 복사 한 번으로 전체 규약 적용 | 룰 변경 시 템플릿을 갱신·재배포해야 함 |
| **대안 1 — 글로벌 룰 승격** | 아래 설명 | △ **부분** — 룰은 자동 적용되나 구조물(문서·폴더)은 미생성 | 가장 강력, 모든 신규 프로젝트에 자동 상속 | 머신·계정에 묶여 팀 공유·이식이 어려움 |
| **대안 2 — 운영 문서(METHODOLOGY.md)** | 아래 설명 | △ **부분** — 체크리스트 문서만 이식, 구조·강제력 없음 | 단일 문서로 즉시 검토·공유 가능 | 강제력이 약하고 폴더 구조가 자동 생성되지 않음 |

> **기존 프로젝트(brownfield) 적용 가능성 검토**: 방법론은 **룰(작업 방식) + 구조물**(결정
> 로그·요구사항 문서·워크스페이스·superpowers 골격)로 이뤄지는데, 이 **둘을 '전체로서'
> 기존 프로젝트에 이식할 수 있는 건 스캐폴드 방식뿐**입니다 — 룰과 구조물을 **실제 파일로
> 함께** 가져와 선별 병합할 수 있기 때문(위 "기존 프로젝트(brownfield)에 적용" 절차 참조).
> - **대안 1(글로벌 룰)**: 룰은 모든 프로젝트에 자동 적용되어 brownfield에도 즉시 걸리지만,
>   요구사항 문서·결정 로그 같은 **구조물은 만들어 주지 않음** → 결국 스캐폴드(또는 수작업) 필요.
> - **대안 2(METHODOLOGY.md)**: 문서 한 장이라 어디든 둘 수 있으나 **구조 생성·강제력이 없어**
>   정보 제공에 그침.
